import 'package:flutter/material.dart';

///Light Theme Colors
/// This color format is different, isn't it? .You can use hex colors here also but you have to remove '#' symball and add 0xff instead.
const Color primaryColor_ = Color.fromARGB(255, 166, 4, 4);
const Color secondaryColor_ = Color(0xFFFFF9E8);
const Color teritoryColor_ = Color(0xFFFBC516);
const Color _backgroundColor = Color(0xFFF6F6F6); //here you can change if you need
const Color _backgroundgreyColor = Color(0x80AFB9C0); //here you can change if you need
const Color textColor = Color(0xFF1B2023);
Color lightTextColor = const Color(0xFF4D5454).withOpacity(0.5);
const Color widgetsBorderColorLight = Color(0x7feeeeee);

///Dark Theme Colors
const Color primaryColorDark = Color(0xff1b2023);
const Color secondaryColorDark = Color(0xff282F39);
const Color teritoryColorDark = Color(0xffFBC516);
const Color backgroundColorDark =
    primaryColorDark; //here you can change if you need
const Color textColorDarkTheme = Color(0xffFDFDFD);
Color lightTextColorDarkTheme = const Color(0xffFDFDFD).withOpacity(0.3);
Color widgetsBorderColorDark = const Color.fromARGB(255, 69, 70, 75);

///Messages Color
const Color errorMessageColor =
    Color.fromARGB(255, 166, 4, 4); // Color(0xffeb5479)
const Color successMessageColor = Color.fromARGB(255, 12, 161, 161);
const Color warningMessageColor = Color(0xFFC2AF6F);

//Button text color
const Color buttonTextColor = Colors.white;

///Advance
//Theme settings
extension ColorPrefs on ColorScheme {
  Color get primaryColor => _getColor(brightness,
      lightColor: primaryColor_, darkColor: primaryColorDark);
  Color get secondaryColor => _getColor(brightness,
      lightColor: secondaryColor_, darkColor: secondaryColorDark);
  Color get teritoryColor => _getColor(brightness,
      lightColor: teritoryColor_, darkColor: teritoryColorDark);

  Color get backgroundColor => _getColor(brightness,
      lightColor: _backgroundColor, darkColor: backgroundColorDark);
  Color get backgroundgreyColor => _getColor(brightness,
      lightColor: _backgroundgreyColor, darkColor: backgroundColorDark);
  Color get buttonColor => buttonTextColor;

  Color get textColorDark => _getColor(brightness,
      lightColor: textColor, darkColor: textColorDarkTheme);

  Color get textLightColor => _getColor(brightness,
      lightColor: lightTextColor, darkColor: lightTextColorDarkTheme);

  Color get borderColor => _getColor(brightness,
      lightColor: widgetsBorderColorLight, darkColor: widgetsBorderColorDark);

  ///This will set text color white if background is dark if background is light it will be dark
  Color get blackColor => Colors.black;

  Color get shimmerBaseColor => brightness == Brightness.light
      ? const Color.fromARGB(255, 225, 225, 225)
      : const Color.fromARGB(255, 150, 150, 150);
  Color get shimmerHighlightColor => brightness == Brightness.light
      ? Colors.grey.shade100
      : Colors.grey.shade300;
  Color get shimmerContentColor => brightness == Brightness.light
      ? Colors.white.withOpacity(0.85)
      : Colors.white.withOpacity(0.7);
}

// 10pt: Smaller
// 12pt: Small
// 16pt: Large
// 18pt: Larger
// 24pt: Extra large
extension TextThemeForFont on TextTheme {
  Font get font => Font();
}

/// i made this to access font easyly from theme like, Theme.of(context).textTheme.font.small
/// So what is diffrence here?? in Theme.of(context).textTheme.small and Theme.of(context).textTheme.font.small
/// We use saperate class because There will be an exention on BuildContext in [Utils/Extensions/lib] folder so further explaination is there. you can check
class Font {
  ///10
  double get smaller => 10;

  ///12
  double get small => 12;

  ///14
  double get normal => 14;

  ///16
  double get large => 16;

  ///18
  double get larger => 18;
  ///20
  double get extraLarge => 24;
  ///24
  double get xLarge => 20;

  ///28
  double get xxLarge => 28;
}

//This one is for check current theme and return data accordingly
_getColor(Brightness brightness,
    {required Color lightColor, required Color darkColor}) {
  if (Brightness.light == brightness) {
    return lightColor;
  } else {
    return darkColor;
  }
}
